...
### Описание исключений
Помимо отдельного ValueError для пустых данных, исключения ловятся через конструкцию try-except и логируются через logger.error(f"*: {str(e)}").
